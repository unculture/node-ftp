...
Parser.parseRecursiveList = function (entries) {
  var returnArray = [
    {
      path: '',
      entries: []
  ...
  for (var x = 0; x < entries.length; x++) {
    ...
    if (XRegExp.exec(entries[x], REX_LISTMSDOS) || XRegExp.exec(entries[x], REX_LISTUNIX)) {
      returnArray[returnArray.length - 1].entries.push(Parser.parseListEntry(entries[x]));
    } else {
        returnArray[returnArray.length - 1].path = entries[x].substr(0, entries[x].length - 1);
    }
  }
